...
int main(void){
    ...
    assert(remove_data(p_list, 0) == LIST_EMPTY);
    ...
    s = remove_end(p_list);
    assert(s == SUCCESS);
    show_list(p_list, "After remvoe_end(): ");

    s = remove_data(p_list, 4);
    assert(s == SUCCESS);
    ...
    printf("get_repeate_count(p_list, 5): %d\n", get_repeate_count(p_list, 5));
    ...
    show_list(p_list, "concateneted version of list1 and list 2: ");
    ...
    show_list(p_list2, "list2 before reversal: ");
    s = reverse_list(p_list2);
    assert(s == SUCCESS);
    show_list(p_list, "list2 after reversal: ");

    s = append_list(p_list1, p_list2);
    assert(s == SUCCESS);
    show_list(p_list1, "list2 is append to list1: ");
...
status_t insert_after(list_t *p_list, data_t e_data, data_t new_data){
    ...
    generic_insert(p_node, get_list_node(e_data), p_node->next);
    return SUCCESS;
}
...
status_t insert_before(list_t *p_list, data_t e_data, data_t new_data){
    ...
    generic_insert(p_node->prev, get_list_node(new_data), p_node);
    return (SUCCESS);
}
...
status_t remove_data(list_t *p_list, data_t r_data){
    ...
    node_t *p_node = NULL;
    p_node = locate_node(p_list, r_data);
...
void show_list(list_t *p_list, const char *msg){
    ...
    puts("[BEG]<->");
    for(p_run = p_list->next; p_list != p_run; p_run = p_run->next){
        printf("[%d]<->", p_run->data);
    }
    puts("[END]");
}
...
status_t reverse_list(list_t *p_list){
    ...
    for(p_run = p_list->prev->prev; p_run != p_list; p_run = p_run->next){
