...
typedef struct dll_node_t dll_list_t;
...
status_t dll_remove_data(dll_list_t *p_list);

bool dll_is_list_empty(dll_list_t *p_list);
bool dll_contains(dll_list_t *p_list);
len_t dll_get_length(dll_list_t *p_list);
int get_repeat_count(dll_list_t *p_list, data_t e_data);
...
void dll_show_list(dll_list_t *p_list);
