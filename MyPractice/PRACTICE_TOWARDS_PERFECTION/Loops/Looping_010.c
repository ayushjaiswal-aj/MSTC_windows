...
void get_subarray(int *pa, int n, int i, int j, int **pp_sub_arr, int *p_n);
void get_reversed_subarray(int *pa, int n, int i, int j, int **pp_reversed_sub_arr, int *p_n);
...
void test_basic_routines(void){
    ...
    for(i=0; i < ARR_SIZE; i = i + 2){
        printf("a[%d]: %d\n", i, a[i]);
    }

    puts("Printing all odd indices -> forward");
    for(i = 1; i < ARR_SIZE; i = i+2){
        printf("a[%d]: %d\n", i, a[i]);
    }
}
...
void test_reverse_routines(void){
    ...
    show(p_ra, ARR_SIZE, "reversed array a: ");
    free(p_ra);
    p_ra = NULL;
...
void get_subarray(int *pa, int n, int i, int j, int **pp_subarray, int *p_n){
    int *p_sub_array;
    int len_sub_array;
    int k, r;
