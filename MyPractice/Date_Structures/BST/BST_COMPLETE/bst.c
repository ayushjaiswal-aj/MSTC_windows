...
ret_t pop_bstptr(stack_t *p_stack, bst_node_t **pp_bst_node){
    return exmaine_and_delete_bstptr_end(p_stack, pp_bst_node);
}
...
ret_t exmaine_and_delete_bstptr_end(list_t *p_list, bst_node_t **pp_bst_node){
    if(is_bstptr_list_empty(p_list)){
        return (LIST_EMPTY);
    }
...
/* bst auxillry - list interface = list auxillary */
