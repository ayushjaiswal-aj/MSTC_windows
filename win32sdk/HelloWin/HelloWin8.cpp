...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd){
    ...
    static TCHAR szCaptionWindow[] = TEXT("GUI - 9");
    ...
    hIcon = LoadIcon((HINSTANCE)NULL, IDI_INFORMATION);
    if(hIcon == NULL){
        MessageBox((HWND)NULL, TEXT("Failed to get handle to icon"), TEXT("Load icon"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }

    hIconSm = LoadIcon((HINSTANCE)NULL, IDI_INFORMATION);
    if(hIconSm == NULL){
