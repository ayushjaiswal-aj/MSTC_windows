...
{
    TCHAR szClassName[] = TEXT("The GUI window");
    TCHAR szWindowCaption[] = TEXT("GUI - 4");
    ...
    hCursor = LoadCursor((HINSTANCE)NULL, IDC_IBEAM);
    if(hCursor == NULL){
    ...
    if(hIconSm == NULL){
        MessageBox((HWND)NULL, TEXT("Failed to get Small Icon handle"), TEXT("LoadIcon"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }
    ...
    if(!RegisterClassEx(&wndEx)){
        MessageBox((HWND)NULL, TEXT("Failed to register window class"), TEXT("RegisterClasssEx"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }
