...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpComLine, int nShowCmd){
    ...
    static TCHAR szWindowCaption[] = TEXT("Window GUI - 3");
    ...
    if(hCursor == NULL){
        MessageBox((HWND)NULL, TEXT("Failed to aquire cursor"), TEXT("LoadCursor"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }

    hIcon = LoadIcon((HINSTANCE)NULL, IDI_APPLICATION);
    if(hIcon == NULL){
        MessageBox((HWND)NULL, TEXT("Failed to aquire icon"), TEXT("LoadIcon"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }
    ...
    if(hWnd == NULL){
        MessageBox((HWND)NULL, TEXT("Failed to aquire window handle"), TEXT("CreateClassEx"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }
