...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd){
    ...
    static TCHAR szWindowCaption[] = TEXT("WindowCaption");
    ...
    if(!RegisterClassEx(&wndEx)){
        MessageBox((HWND)NULL, TEXT("Failed to register class for window"), TEXT("RegisterClassEx"), MB_ICONERROR);
        return (EXIT_FAILURE);
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
    ...
    switch(uMsg){
        ...
        case WM_PAINT:
            ...
            TextOut(hdc, cxClient/4, cyClient/4, ppWindowPaint[0], lstrlen(ppWindowPaint[0]));
            
            SetTextAlign(hdc, TA_RIGHT | TA_CENTER);
            TextOut(hdc, (cxClient * 3)/4, cyClient/4, ppWindowPaint[0], lstrlen(ppWindowPaint[0]));
            
            SetTextAlign(hdc, TA_RIGHT | TA_CENTER);
            TextOut(hdc, cxClient/4, (cyClient * 3)/4, ppWindowPaint[0], lstrlen(ppWindowPaint[0]));
            
            SetTextAlign(hdc, TA_LEFT | TA_CENTER);
            TextOut(hdc, (cxClient * 3)/4, (cyClient * 3)/4, ppWindowPaint[0], lstrlen(ppWindowPaint[0]));
